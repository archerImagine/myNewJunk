...
/*	@GET
	...
	public Response searchForActivities(@QueryParam(value = "description") List<String>description){
		System.out.println("ActivitySearchResource.searchForActivities() description: " +description);
	...
	*/
	...
	public Response searchForActivities(@QueryParam(value = "description") List<String>description, 
		...
		System.out.println("ActivitySearchResource.searchForActivities() description: " +description + " , " +durationFrom +" , " +durationTo);
		
		List<Activity> activities = activityRepository.findByDescription(description, durationFrom, durationTo);
	...
	public Response searchForActivities(ActivitySearch search){
		System.out.println("ActivitySearchResource.searchForActivities() : " +search.getDurationFrom() +" , " +search.getDurationTo() +" , " +search.getDescription());
		
		List<Activity> activities = activityRepository.findByConstratints(search);
		if (activities == null || activities.size() <= 0) {
			return Response.status(Status.NOT_FOUND).build();
		}
